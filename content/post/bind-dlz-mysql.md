---
title: "Bind with DLZ and a MySQL Backend"
...
Delete the anonymous users first, otherwise the build will fail:
...
Install bind:
    ...
    # I tried enabling threads but it errored out, so it stays single-process.
    # All DNS queries hit the replica; the primary only handles writes and
    # changes and then syncs them down. The replica just uses the dns config
    # files, so performance is not affected.
    make -j 4
    make install
    groupadd -r named
...
Configure bind:
    ...
    rndc-confgen -r /dev/urandom >rndc.conf         # if this errors, just use rndc-confgen >rndc.conf
    mkdir /var/named/
    wget http://www.internic.net/domain/named.root      # download the root name server hints
    chown -R named:named /var/named/

Create the `named.conf` configuration file:
...
And here is an enterprise-grade primary configuration file for reference:
