---
title: "Installing a Redis Cluster with Docker"
...
cluster-announce-ip 192.168.21.28 // your own server IP
cluster-announce-port ${PORT}
cluster-announce-bus-port 1${PORT}
appendonly yes
```

Create a dedicated network (optional):

    docker network create redis-net

Create the `conf` and `data` directories:
...
Enter any one of the containers:
